...
router.post('/api/auth/register', async (req, res) => {
  ...
  // use unique identifier (email) to create JWT token
  const token = jwt.sign({ id: savedUser.email }, JWT_SECRET, { expiresIn: '1h' });

  res.status(201).json({ message: 'User registered successfully', user: savedUser, token: `Bearer ${token}` });
});
